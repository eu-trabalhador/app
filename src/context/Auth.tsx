...
async function signIn(email: string, password: string): Promise<void> {
  ...
  const unsubscribe = auth().onAuthStateChanged(async (_user) => {
    if (_user) {
      console.log("Usuário restaurado automaticamente pelo Firebase:", _user.email);
      setUser(_user);
      setAction("login");

      // mantém dados básicos no AsyncStorage
      ...
      console.log("Nenhum usuário logado. Limpando sessão local.");
      await AsyncStorage.removeItem("@user");
      setUser(null);
      setUsuario({} as UsuarioType);
      setLoged(false);
